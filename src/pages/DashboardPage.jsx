...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen pt-20 bg-lt-navy">
      <div className="bg-lt-surface border-b border-lt-surface-border py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="font-display font-bold text-4xl text-white uppercase tracking-wide">Fleet Dashboard</h1>
          </div>
          <div className="flex gap-1 p-1 bg-lt-surface-card border border-lt-surface-border rounded-xl">
            {[['fleet','Fleet Owner'],['operator','Operator'],['supervisor','Supervisor']].map(([id,label]) => (
              <button key={id} onClick={() => setRole(id)}
                className={`px-4 py-2 rounded-lg text-xs font-body font-semibold transition-all
                  ${role===id ? 'bg-lt-red text-white' : 'text-lt-gray-muted hover:text-white'}`}>
                {label}
              </button>
            ))}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            ...
            <div key={k.label} className="card-dark">
              <div className="flex items-center justify-between mb-3">
                <k.icon size={16} className={k.color}/>
                <Bell size={13} className="text-lt-gray-muted"/>
              </div>
              ...
              <div className="text-xs text-lt-gray-muted font-body">{k.sub}</div>
            </div>
          ))}
        </div>
        ...
        <div className="card-dark mb-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <span className="text-xs text-lt-gray-muted font-body">{MACHINES.length} machines</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm font-body">
              <thead>
                <tr className="border-b border-lt-surface-border text-left">
                  {['Machine','Serial','Site','Health','Hours','Next PM','Status','Action'].map(h => (
                    <th key={h} className="text-xs text-lt-gray-muted uppercase tracking-wider pb-3 pr-4 whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {MACHINES.map(m => (
                  <tr key={m.id} className="border-b border-lt-surface-border/50 last:border-0 hover:bg-white/2 transition-colors">
                    <td className="py-3 pr-4 font-display font-bold text-white text-base">{m.model}</td>
                    <td className="py-3 pr-4 font-mono text-xs text-lt-gray-muted">{m.serial}</td>
                    <td className="py-3 pr-4 text-gray-300 text-xs whitespace-nowrap">{m.site}</td>
                    ...
                    <td className="py-3 pr-4 text-gray-300 text-xs">{m.hours.toLocaleString()}</td>
                    <td className="py-3 pr-4">
                      <span className={`text-xs font-bold ${m.nextPM==='Overdue'?'text-lt-red':m.nextPM==='—'?'text-lt-gray-muted':'text-gray-300'}`}>{m.nextPM}</span>
                    </td>
                    <td className="py-3 pr-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div className="card-dark">
            ...
            {[['PC210 — Pune Site','FMC/MARC','Expires Dec 2025','Active'],['D155 — Nagpur Mine','CMC','Expires Mar 2026','Active'],['HD465 — Singrauli','SSA','Expires Aug 2025','Renewal Due']].map(([m,type,exp,st]) => (
              <div key={m} className="flex items-center justify-between py-2.5 border-b border-lt-surface-border/50 last:border-0">
                <div><div className="text-sm font-body text-white">{m}</div><div className="text-xs text-lt-gray-muted">{type} · {exp}</div></div>
          ...
          <div className="card-dark">
            ...
            {[['Engine Oil Filter','PC210','In Stock','✅'],['Final Drive Gear','HD465','2 Weeks','⏳'],['Bucket Tooth Set','PC350','In Stock','✅'],['Air Filter Element','D155','3 Days','🔄']].map(([part,m,avail,icon]) => (
              <div key={part} className="flex items-center justify-between py-2.5 border-b border-lt-surface-border/50 last:border-0">
                <div><div className="text-sm font-body text-white">{part}</div><div className="text-xs text-lt-gray-muted">{m}</div></div>
                <span className="text-xs text-gray-300 font-body">{icon} {avail}</span>
              </div>
            ))}
          </div>
        </div>
