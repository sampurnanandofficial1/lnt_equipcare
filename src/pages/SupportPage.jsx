...
const SEV = { Critical:'text-lt-red bg-lt-red-pale border-lt-red/30', Warning:'text-amber-700 bg-amber-50 border-amber-200', Info:'text-blue-700 bg-blue-50 border-blue-200' }
    ...
    <div className="min-h-screen bg-white" style={{ paddingTop:'100px' }}>
      <div className="bg-lt-navy py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="section-label mb-2 text-lt-red">Self-Help Hub</div>
          ...
          <div className="flex gap-3">
            <button onClick={onChatOpen} className="btn-red flex items-center gap-2"><Bot size={14}/> Ask AI First</button>
          ...
          {[['troubleshoot','Troubleshoot'],['errorcodes','Error Codes'],['manuals','Manuals']].map(([id,label]) => (
            <button key={id} onClick={() => setTab(id)}
              className={`px-5 py-3 text-sm font-body font-medium border-b-2 transition-all -mb-px
                ${tab===id ? 'border-lt-red text-lt-red' : 'border-transparent text-lt-gray hover:text-lt-navy'}`}>{label}</button>
          ))}
        </div>
        {tab==='troubleshoot' && (
            ...
            {CATS.map(c => (
              <div key={c.id} className="card p-5">
                ...
                <ul className="space-y-1">
                  {c.issues.map(issue => (
                    <li key={issue}><button className="w-full text-left flex items-center justify-between text-sm text-lt-gray-dark hover:text-lt-red py-2 border-b border-lt-gray-border last:border-0 font-body group">
                      {issue}<ChevronRight size={12} className="text-lt-gray group-hover:text-lt-red"/>
                    </button></li>
                  ))}
                </ul>
                <button onClick={onChatOpen} className="w-full mt-4 py-2 text-xs font-body font-semibold text-lt-red border border-lt-red rounded hover:bg-lt-red-pale transition-all">Diagnose with AI →</button>
              </div>
            ))}
          </div>
        )}
          ...
          <div>
            <div className="relative mb-6 max-w-md">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search error code (e.g. E0110)"
                className="w-full pl-10 pr-4 py-2.5 border border-lt-gray-border rounded-lg text-sm text-lt-navy placeholder-lt-gray font-body focus:outline-none focus:border-lt-red transition-colors"/>
            </div>
            <div className="space-y-3">
              {filtered.map(e => (
                <div key={e.code} className="card p-4 flex flex-col sm:flex-row sm:items-start gap-4">
                  <div className="flex-shrink-0"><span className="font-mono font-bold text-xl text-lt-red">{e.code}</span><div className="text-xs text-lt-gray font-body">{e.system}</div></div>
                  ...
                  <button onClick={onChatOpen} className="text-xs text-lt-red hover:underline font-body flex-shrink-0">AI Help →</button>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[['PC200/205/210 Operation Manual','Hydraulic Excavator','PDF · 12MB'],['PC300/350 Operation Manual','Hydraulic Excavator','PDF · 15MB'],['D85 Dozer Manual','Crawler Dozer','PDF · 9MB'],['GD705 Motor Grader Manual','Motor Grader','PDF · 11MB'],['HD465 Dump Truck Manual','Mining Dump Truck','PDF · 18MB'],['WA470 Wheel Loader Manual','Wheel Loader','PDF · 8MB']].map(([t,tp,sz]) => (
              <div key={t} className="card p-4 flex items-start gap-3">
                <div className="w-10 h-12 bg-lt-red-pale border border-lt-red/20 rounded flex items-center justify-center flex-shrink-0">
                ...
                <button className="p-1.5 border border-lt-gray-border rounded hover:border-lt-red hover:text-lt-red text-lt-gray transition-all flex-shrink-0"><Download size={13}/></button>
              </div>
            ))}
          </div>
        )}
