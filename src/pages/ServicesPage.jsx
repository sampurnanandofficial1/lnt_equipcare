...
export default function ServicesPage() {
  ...
  const inputCls = "w-full bg-lt-surface border border-lt-surface-border rounded-xl px-4 py-2.5 text-sm text-white placeholder-lt-gray-muted font-body focus:outline-none focus:border-lt-red/50 transition-colors"
    ...
    <div className="min-h-screen pt-20 bg-lt-navy">
      <div className="bg-lt-surface border-b border-lt-surface-border py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="section-label mb-2">Service & Support</div>
          <h1 className="section-title mb-3">Service Management</h1>
          <p className="text-lt-gray-muted font-body max-w-xl">Book service, track AMC, manage warranties & raise breakdown requests — all in one place.</p>
        </div>
      </div>
            ...
            <div className="card-dark">
              ...
              {submitted ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-lt-gray-muted font-body mb-4">Your service ticket has been created.</p>
                  ...
                  <p className="text-xs text-lt-gray-muted font-body mt-4">Our team will contact you within 2–4 hours.</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div><label className="text-xs text-lt-gray-muted font-body mb-1.5 block uppercase tracking-wider">Contact Name *</label>
                    <input value={form.name} onChange={set('name')} placeholder="Your name" className={inputCls}/></div>
                  <div><label className="text-xs text-lt-gray-muted font-body mb-1.5 block uppercase tracking-wider">Company / Site Name</label>
                    <input value={form.company} onChange={set('company')} placeholder="Company or project site" className={inputCls}/></div>
                  <div><label className="text-xs text-lt-gray-muted font-body mb-1.5 block uppercase tracking-wider">Phone Number *</label>
                    <input value={form.phone} onChange={set('phone')} placeholder="+91 XXXXX XXXXX" className={inputCls}/></div>
                  <div><label className="text-xs text-lt-gray-muted font-body mb-1.5 block uppercase tracking-wider">Machine Type *</label>
                    <select value={form.machine} onChange={set('machine')} className={selectCls}>
                      <option value="">Select machine...</option>
                      {MACHINE_TYPES.map(m => <option key={m}>{m}</option>)}
                    </select></div>
                  <div><label className="text-xs text-lt-gray-muted font-body mb-1.5 block uppercase tracking-wider">Serial / PIN Number</label>
                    <input value={form.serial} onChange={set('serial')} placeholder="Machine serial number" className={inputCls}/></div>
                  <div><label className="text-xs text-lt-gray-muted font-body mb-1.5 block uppercase tracking-wider">Service Required *</label>
                    <select value={form.serviceType} onChange={set('serviceType')} className={selectCls}>
                      <option value="">Select service type...</option>
                      {SERVICE_TYPES.map(s => <option key={s}>{s}</option>)}
                    </select></div>
                  <div className="sm:col-span-2"><label className="text-xs text-lt-gray-muted font-body mb-1.5 block uppercase tracking-wider">Zone / Region *</label>
                    <select value={form.zone} onChange={set('zone')} className={selectCls}>
                      <option value="">Select your zone...</option>
                      {ZONES.map(z => <option key={z}>{z}</option>)}
                    </select></div>
                  <div className="sm:col-span-2"><label className="text-xs text-lt-gray-muted font-body mb-1.5 block uppercase tracking-wider">Issue Description</label>
          ...
          <div className="space-y-5">
            <div className="card-dark">
              ...
              {[['Breakdown','2–4 hours','Critical'],['PM Service','24 hours','Normal'],['Component Overhaul','48 hours','Planned'],['AMC Renewal','Same day','Admin']].map(([t,r,p]) => (
                <div key={t} className="flex items-center justify-between py-2 border-b border-lt-surface-border/50 last:border-0">
                  <div><div className="text-sm font-body text-white">{t}</div><div className="text-xs text-lt-gray-muted">{p}</div></div>
            ...
            <div className="card-dark">
              ...
              {['FMC / MARC','CMC','Cost Cap','SSA','GPC'].map(a => (
                <div key={a} className="flex items-center gap-2 py-1.5 border-b border-lt-surface-border/50 last:border-0">
            ...
            <div className="p-4 rounded-xl bg-lt-red/5 border border-lt-red/20">
              <div className="text-xs font-body font-bold text-lt-red uppercase tracking-wider mb-2">Emergency Breakdown?</div>
              <p className="text-xs text-lt-gray-muted font-body mb-3">Call Equipcare directly for immediate field dispatch.</p>
