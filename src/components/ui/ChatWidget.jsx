...
const QUICK = ['Book a service request','Check AMC contract types','Find nearest service center','Explain warning codes','Spare parts enquiry','Machine breakdown support']
...
export default function ChatWidget({ open, onClose }) {
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-[100] flex flex-col transition-all duration-300 rounded-2xl overflow-hidden
                     shadow-[0_20px_60px_rgba(0,0,0,0.20)] border border-lt-gray-border
                     ${minimized ? 'h-14 w-80' : 'h-[580px] w-[370px] max-h-[90vh]'}`}>

      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 bg-lt-red">
        <div className="flex items-center gap-2.5">
          <div className="relative">
            <div className="w-8 h-8 bg-white/20 rounded-full flex items-center justify-center">
              <Bot size={16} className="text-white"/>
            </div>
            <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-green-400 rounded-full border-2 border-lt-red"/>
          </div>
          <div>
            <div className="text-sm font-display font-bold text-white uppercase tracking-wide">Equipcare AI</div>
            <div className="text-[10px] text-red-200 font-body">Online · 24×7 Support</div>
          </div>
        </div>
        <div className="flex items-center gap-1">
          <a href="tel:18008339990" className="p-1.5 rounded text-red-200 hover:text-white hover:bg-white/15 transition-all" title="Call Equipcare">
            <Phone size={13}/>
          </a>
          <button onClick={() => setMin(!minimized)} className="p-1.5 rounded text-red-200 hover:text-white hover:bg-white/15 transition-all">
            {minimized ? <Maximize2 size={13}/> : <Minimize2 size={13}/>}
          </button>
          <button onClick={onClose} className="p-1.5 rounded text-red-200 hover:text-white hover:bg-white/15 transition-all">
            <X size={13}/>
          </button>
        </div>
      </div>
      ...
      {!minimized && (
        <>
          ...
          <div className="flex-1 overflow-y-auto bg-lt-gray-bg px-3 py-4 space-y-4">
            {messages.map((m,i) => (
              <div key={i} className={`flex gap-2.5 ${m.role==='user' ? 'flex-row-reverse' : ''}`}>
                <div className={`w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center mt-0.5 ${m.role==='assistant' ? 'bg-lt-red' : 'bg-lt-navy'}`}>
                ...
                <div className={`max-w-[80%] px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed font-body
                  ${m.role==='assistant' ? 'bg-white border border-lt-gray-border text-lt-navy rounded-tl-sm shadow-sm' : 'bg-lt-red text-white rounded-tr-sm'}`}
            ...
            {error && (
              <div className="flex items-start gap-2 px-3 py-2.5 bg-red-50 border border-red-200 rounded-xl text-xs text-lt-red">
          ...
          {/* Quick replies */}
          {messages.length <= 1 && (
            <div className="bg-white px-3 pb-2 pt-2 border-t border-lt-gray-border">
              <div className="text-[10px] text-lt-gray font-body mb-2 uppercase tracking-wider">Quick Actions</div>
              <div className="flex flex-wrap gap-1.5">
                {QUICK.map(q => (
                  <button key={q} onClick={() => send(q)}
                    className="text-xs px-2.5 py-1 rounded-full border border-lt-gray-border text-lt-gray-dark bg-lt-gray-bg hover:border-lt-red hover:text-lt-red hover:bg-lt-red-pale transition-all">
                    {q}
                  </button>
                ))}
              </div>
          ...
          <div className="p-3 bg-white border-t border-lt-gray-border">
            <div className="flex gap-2 items-end">
              <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)}
                ...
                rows={1} className="flex-1 border border-lt-gray-border rounded-xl px-3.5 py-2.5 text-sm text-lt-navy
                                    placeholder-lt-gray font-body resize-none focus:outline-none focus:border-lt-red transition-colors"
